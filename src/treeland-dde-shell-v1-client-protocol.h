 ...
 * @ingroup iface_treeland_dde_shell_surface_v1
 *
 * Place the surface so that its origin is offset by (x_offset, y_offset)
 * from the cursor. The client passes a negative offset equal to its shadow
 * margin so the visible panel lands exactly on the cursor.
 *
 ...
 */
static inline void
treeland_dde_shell_surface_v1_set_auto_placement(struct treeland_dde_shell_surface_v1 *treeland_dde_shell_surface_v1, int32_t x_offset, int32_t y_offset)
{
	wl_proxy_marshal_flags((struct wl_proxy *) treeland_dde_shell_surface_v1,
			 TREELAND_DDE_SHELL_SURFACE_V1_SET_AUTO_PLACEMENT, NULL, wl_proxy_get_version((struct wl_proxy *) treeland_dde_shell_surface_v1), 0, x_offset, y_offset);
}
